...
//		mutex_lock_killable(&m);
		...
		"%04d/%02d/%02d %02d:%02d:%02d, %s (pid %i) is opening %s\n",
...
//		mutex_lock_killable(&m);
		...
		snprintf(line_to_add->msg, MAX_HISTORY_LINE,
		"%04d/%02d/%02d %02d:%02d:%02d, %s (pid %i) is reading %d bytes from TODO\n",
		tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec,
		p, current->pid, (int)count);
		line_to_add->time_in_sec = (u32)time.tv_sec;
...
//		mutex_lock_killable(&m);
		...
		snprintf(line_to_add->msg, MAX_HISTORY_LINE,
		"%04d/%02d/%02d %02d:%02d:%02d, %s (pid %i) is writing %d bytes to TODO\n",
		tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec,
		p, current->pid, (int)count);
		line_to_add->time_in_sec = (u32)time.tv_sec;
